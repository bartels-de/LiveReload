...
{
    public sealed class ResponseStreamWrapper : Stream
    {
        ...
        public override Task FlushAsync(CancellationToken cancellationToken)
        {
            ...
            if (_isContentLengthSet || !IsHtmlResponse()) return _baseStream.FlushAsync(cancellationToken);
        ...
        public override async ValueTask WriteAsync(ReadOnlyMemory<byte> buffer, CancellationToken cancellationToken = default)
        {
            ...
            {
                if (_baseStream != null) await _baseStream.WriteAsync(buffer, cancellationToken);
            }
        }

        private bool IsHtmlResponse(bool forceReCheck = false)
        {
            if (!forceReCheck && _isHtmlResponse != null) return _isHtmlResponse.Value;
            ...
            if (_context?.Response == null) return false;
            ...
            _isHtmlResponse = _context.Response?.Body != null && (_context.Response.StatusCode == 200 || _context.Response.StatusCode == 500) &&
            ...
            if (!_isHtmlResponse.Value) return false;

            // Check for refresh exlusion rules
            var refreshFile = RefreshInclusionModes.ContinueProcessing;
            if (LiveReloadConfiguration.Current.RefreshInclusionFilter != null)
            {
                ...
                if (refreshFile == RefreshInclusionModes.DontRefresh) return false; // don't embed refresh script
            }
        ...
        {
            if (buffer.Length == 0 || bufferToFind.Length == 0) return -1;
        ...
        public static int IndexOfByteArray(byte[] buffer, string stringToFind, Encoding encoding = null)
        {
            ...
            if (buffer.Length == 0 || string.IsNullOrEmpty(stringToFind)) return -1;
